...
Public Class DeleteMarks
    ...
    Private Sub comSubjectId_SelectedIndexChanged(sender As Object, e As EventArgs) Handles comSubjectId.SelectedIndexChanged
        Dim da As New SqlDataAdapter(New SqlCommand("SELECT * FROM marks where studentId = '" & comStudentId.SelectedItem & "' AND subjectId = '" & comSubjectId.SelectedItem & "';", Form2.connection))
        Dim datatable As New DataTable()
        da.Fill(datatable)

        txtExamYear.Text = datatable.Rows(0).Item(2)
        txtMark.Text = datatable.Rows(0).Item(3)
    End Sub
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        Dim proceedDelete As Integer = MessageBox.Show("Mark record of student with student id " & studentId & " for subject with subject id " & subjectId & " will be deleted", "Delete Mark Record", MessageBoxButtons.YesNo, MessageBoxIcon.Warning)
        If proceedDelete = 6 Then
            Dim insertQuery As String = "delete from marks where studentId = '" & studentId & "' AND subjectId = '" & subjectId & "' ;"
            Form2.run_query(insertQuery)
            Form2.table_load("marks")

            MessageBox.Show("Mark record of student with student id " & studentId & " for subject with subject id " & subjectId & " has been deleted", "Mark Record Deleted", MessageBoxButtons.OK, MessageBoxIcon.Information)
            load_StudentIdCombobox()
        End If
    End Sub
End Class